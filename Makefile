...
$(AUTOLOADS): $(LISP_FILES) $(SOURCE_LIST) $(BUILD_FILE)
	@set -eu
	temp_dir=$$(mktemp -d)
...
$(ELC_FILES) &: $(COMPILE_DEPS) $(SOURCE_LIST) $(BUILD_FILE)
	$(EMACS_BATCH) \
...
$(ARCHIVE_STAMP): $(LISP_FILES) $(PKG) $(INFO) $(SOURCE_LIST) COPYING $(BUILD_FILE)
	@set -eu
	version=$$(env \
		PACKAGE_SOURCE="$(MAIN)" \
		$(EMACS_BATCH) \
